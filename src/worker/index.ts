...
export async function startWorker(env: NodeJS.ProcessEnv = process.env,
  ...
  const worker = createWorker({ ...options,
    ...
    ...(env['TYPESAFE_API_KEY'] ? { decisions: { apiKey: env['TYPESAFE_API_KEY'], model: env['JEV_MODEL'] ?? 'jev-1.13.0', mode: (env['JEV_MODE'] ?? 'active') as import('../model/decision.js').DecisionMode } } : {}),
