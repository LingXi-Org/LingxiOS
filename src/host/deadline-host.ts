...
const signalIndex: Record<keyof HostPort, number> = { claimWork: 0, recoverWork: 1, waitForWork: 2, streamPreview: 2, heartbeat: 1, loadContext: 1, loadInitialContext: 1, reserveModelCall: 3,
  prepareToolDecision: 2, prepareMemoryReview: 2,recordMemoryReview: 4, loadWorkspace: 1, stageWorkspaceFile: 3, readWorkspaceFile: 2,
...
export function deadlineHost(host: HostPort, signal?: AbortSignal, timeoutMs = 30_000, metrics?: MetricsRegistry): HostPort {
  return new Proxy(host, { get(target, property, receiver) {
    ...
    return (...args: unknown[]) => {
      ...
      const bounded = AbortSignal.any([...property === 'streamPreview' ? [] : [AbortSignal.timeout(property === 'recordModelUsage' ? 5000
        : ['stageWorkspaceFile', 'readWorkspaceFile', 'saveStep'].includes(String(property)) ? 300_000 : timeoutMs)],
