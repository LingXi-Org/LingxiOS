...
```sh
npm install @lyyzka/lingxios@5.0.0-rc.2
```
...
Optional `control.memory` provides scoped Markdown documents, always-loaded core memory, Chinese/English PostgreSQL search, committed history, versioned edits, background reflection, diagnostics and rollback. Version 5.0.0-rc.2 requires schema 11 and protocol 13, including distributed workspace checkpoints. Schema-9 installations apply `packageResources().migration010`, then `migration011` and `migration012` and `migration013` (schema-10 installations need migrations 011-013); existing memory and business records remain intact. IM conversation policy, multi-Agent reply slots, durable DAGs and field-versioned shared state are available through the [IM collaboration API](docs/im-collaboration.md). See the [memory configuration and cutover procedure](docs/packaged-runtime.md#cognitive-memory).

## Jev semantic decisions

Pass `decisions: { apiKey: process.env.TYPESAFE_API_KEY, model: 'jev-1.13.0' }` to `createWorker`. The standalone worker reads `TYPESAFE_API_KEY`, `JEV_MODEL` and `JEV_MODE` (default `active`). No key leaves existing generative reviewers unchanged. `active` uses Jev first and falls back once on uncertainty/provider failure. Explicit rejection, cancellation, exhausted budgets, lease loss and persistence errors never trigger an extra paid fallback. Memory proposals and user-facing generation remain generative.

`modes` and `thresholds` override individual purposes: `memory-write-review`, `memory-synthesis-verification`, `content-review`, `memory-relevance`, `memory-durability`, `route-selection`, and product-defined purposes. `off` skips Jev; `shadow` records decisions without adopting them; `active` uses clear decisions. Confidence is not correctness probability. Defaults are 0.8 for ranking/routing and 0.95 for review, pending domain calibration. Oversized state falls back without truncating requirements. No automatic Jev retries.
...
Protocol 13 adds private Worker tool decision preparation. `ToolDefinition.prepareDecision` supplies authorized state; the normal action path rechecks a digest covering principal, action, source content, request version, fence and tool semantics before exposing `ActionContext.decision`. Both native and Python calls share this path. No inference holds a write transaction. Bump `semanticVersion` when review semantics change. `runtime.decision` and comparison steps retain structured labels and digests, not prompts. Drain existing work before upgrading; schema stays at 11. Do not mix protocol versions or rewrite active run bindings. Package tests do not establish semantic quality.
